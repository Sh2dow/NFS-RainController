...
static bool GetEViewMatrixMW(D3DXMATRIX& outView)
{
    ...
    if (!view || !core::IsReadable(view, sizeof(void*)))
        return false;
    void* viewObj = *reinterpret_cast<void**>(view); // sub_6CF400 "this"
    if (!viewObj || !core::IsReadable(viewObj, sizeof(D3DXMATRIX)))
        return false;
    const auto tryOffset = [&](uintptr_t offset, D3DXMATRIX& candidate) -> bool
    {
        auto* ptr = reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(viewObj) + offset);
    ...
    D3DXMATRIX m0{}, m1{}, m2{}, m3{};
    bool ok0 = tryOffset(WeatherGameAddresses::EViewMatrixOffset0_MW, m0);
    bool ok1 = tryOffset(WeatherGameAddresses::EViewMatrixOffset1_MW, m1);
    bool ok2 = tryOffset(WeatherGameAddresses::EViewMatrixOffset0_MW + 0x80, m2);
    bool ok3 = tryOffset(WeatherGameAddresses::EViewMatrixOffset1_MW + 0x80, m3);
    const auto pickView = [&](const D3DXMATRIX& m) { return !IsProjectionLike(m); };
    if (ok0 && pickView(m0)) { outView = m0; return true; }
    if (ok1 && pickView(m1)) { outView = m1; return true; }
    if (ok2 && pickView(m2)) { outView = m2; return true; }
    if (ok3 && pickView(m3)) { outView = m3; return true; }
    if (ok0) { outView = m0; return true; }
    if (ok1) { outView = m1; return true; }
    if (ok2) { outView = m2; return true; }
    if (ok3) { outView = m3; return true; }
    return false;
}
...
static bool GetProjFromEViewMatrixMW(void* viewPtr, D3DXMATRIX& outProj)
{
    if (!viewPtr)
        return false;
    if (!core::IsReadable(viewPtr, sizeof(void*)))
        return false;
    void* viewObj = *reinterpret_cast<void**>(viewPtr);
    if (!viewObj)
        return false;
    const uintptr_t base = reinterpret_cast<uintptr_t>(viewObj);
    const uintptr_t offsets[] = {0x40, 0x80, 0xC0, 0x100};
    for (uintptr_t off : offsets)
    {
        ...
        if (!core::IsReadable(mat, sizeof(D3DXMATRIX)))
            continue;
        D3DXMATRIX m = *mat;
        if (IsProjectionLike(m))
        {
            outProj = m;
            return true;
        }
    }
    // Fallback: first readable matrix
    for (uintptr_t off : offsets)
    {
        auto* mat = reinterpret_cast<D3DXMATRIX*>(base + off);
        if (!core::IsReadable(mat, sizeof(D3DXMATRIX)))
            continue;
        outProj = *mat;
        return !IsIdentityMatrix(outProj);
    }
    return false;
}
...
D3DXVECTOR3 PrecipitationController::GetCameraPositionSafe()
{
    if (detected_game == GameType::MW)
    {
        ...
        {
            OutputDebugStringA("[GetCameraPositionSafe] MW viewPtr unreadable\n");
            loggedViewPtrOnce = true;
        }
        if (m_device)
        {
...
void PrecipitationController::Render3DRainOverlay(const D3DVIEWPORT9& viewport)
{
    ...
    if (!renderEntryLogged)
    {
        ...
        {
            OutputDebugStringA("[RainDebug MW] Render3D entry\n");
    ...
    if (detected_game == GameType::MW)
    {
        camPtr = PrecipitationController::Get()->m_cameraPtr;
        if (!camPtr)
            camPtr = GetCameraPtrFromEView();
        bool selected = false;
        // Prefer camera params for MW world-space (view/proj derived from camera)
        void* viewPtr = g_mwActiveViewPtr;
        if (!viewPtr)
            viewPtr = *reinterpret_cast<void**>(WeatherGameAddresses::EViewCurrentPtr_MW);
        void* camParams = GetCameraParamsFromEView();
        if (camParams && GetViewFromCameraParams(camParams, matView) &&
            GetProjFromCameraParams(camParams, viewport, matProj))
        {
            DebugLogMatrixSourceOnce("Render3D camParams forced");
            DebugLogMatrixOnce("Render3D camParams view", matView);
            DebugLogMatrixOnce("Render3D camParams proj", matProj);
            hasProj = true;
            selected = true;
        }
        else if (GetEViewMatrixMW(matView))
        {
            DebugLogMatrixSourceOnce("Render3D eView forced");
            DebugLogMatrixOnce("Render3D eView", matView);
            if (GetProjFromEView(viewPtr, viewport, matProj))
                hasProj = true;
            selected = true;
        }
        // Prefer MW rain matrices (matches Rain::Render view/proj usage).
        if (!selected && GetRainMatricesMW(matView, matProj))
        {
            usedRainMatrices = true;
            hasProj = true;
            DebugLogMatrixSourceOnce("Rain matrices (forced)");
        ...
        else if (!selected && GetEViewMatrixMW(matView))
        {
            ...
            // Prefer MW camera params when available (matches sub_6CF400).
            if (camPtr)
            {
                void* camPtrNorm = NormalizeCameraPtr(camPtr);
                bool hasCamView = GetViewFromCameraStruct(camPtrNorm, matView);
                if (!hasCamView)
                    hasCamView = GetViewFromCameraParams(camPtrNorm, matView);
                if (hasCamView && GetProjFromCameraParams(camPtrNorm, viewport, matProj))
                {
                    hasProj = true;
                    DebugLogMatrixSourceOnce("Render3D camera params forced");
                    DebugLogMatrixOnce("Render3D camView", matView);
                    DebugLogMatrixOnce("Render3D camProj", matProj);
                }
            }

            if (!hasProj && IsProjectionLike(matView))
            {
                void* viewPtr = g_mwActiveViewPtr;
                if (!viewPtr)
                    viewPtr = *reinterpret_cast<void**>(WeatherGameAddresses::EViewCurrentPtr_MW);
                ...
                void* camPtrNorm = NormalizeCameraPtr(camPtr);
                if (GetViewProjFromCameraParams(camPtrNorm, viewport, viewFromParams, projFromParams))
                {
                    matView = viewFromParams;
                    matProj = projFromParams;
                    hasProj = true;
                    DebugLogMatrixSourceOnce("Render3D camera params view/proj");
                    DebugLogMatrixOnce("Render3D camView", matView);
                }
                else
                {
                    DebugLogMatrixSourceOnce("Render3D camera params failed");
                    useViewProjOnly = true;
                    matViewProj = matView;
                    transposeVP = true;
                    DebugLogMatrixSourceOnce("Render3D view-proj only");
                    DebugLogMatrixOnce("Render3D VP", matViewProj);
                }
            }
            else if (!hasProj)
            {
                if (GetProjFromCameraParams(NormalizeCameraPtr(camPtr), viewport, matProj))
                {
                    hasProj = true;
                    DebugLogMatrixSourceOnce("Render3D eView proj");
                    DebugLogMatrixNoGuard("Render3D proj", matProj);
                }
                else
                {
...
void PrecipitationController::Render3DSplattersOverlay(const D3DVIEWPORT9& viewport)
{
    ...
    if (!splatEntryLogged)
    {
        ...
        {
            OutputDebugStringA("[RainDebug MW] Splatters entry\n");
    ...
    if (detected_game == GameType::MW)
    {
        ...
        else if (!selected && GetEViewMatrixMW(matView))
        {
            ...
            // Prefer MW camera params when available (matches sub_6E44C0).
            if (camPtr)
            {
                void* camPtrNorm = NormalizeCameraPtr(camPtr);
                bool hasCamView = GetViewFromCameraStruct(camPtrNorm, matView);
                if (!hasCamView)
                    hasCamView = GetViewFromCameraParams(camPtrNorm, matView);
                if (GetProjFromCameraParams(camPtrNorm, viewport, matProj))
                {
                    hasProj = true;
                    DebugLogMatrixSourceOnce("Splatters camera params forced");
                    DebugLogMatrixOnce("Splatters camView", matView);
                }
            }
            ...
            if (!hasProj && IsProjectionLike(matView))
            {
                ...
                if (GetViewProjFromCameraParams(camPtrNorm, viewport, viewFromParams, projFromParams))
                {
                    ...
                    matProj = projFromParams;
                    hasProj = true;
                    DebugLogMatrixSourceOnce("Splatters camera params view/proj");
                    DebugLogMatrixOnce("Splatters camView", matView);
                }
                else
                {
                    DebugLogMatrixSourceOnce("Splatters camera params failed");
                    useViewProjOnly = true;
                    matViewProj = matView;
                    transposeVP = true;
                    DebugLogMatrixSourceOnce("Splatters view-proj only");
                    DebugLogMatrixOnce("Splatters VP", matViewProj);
                }
            }
            else if (!hasProj)
            {
                ...
                {
                    hasProj = true;
                    DebugLogMatrixSourceOnce("Splatters eView proj");
                    DebugLogMatrixNoGuard("Splatters proj", matProj);
                }
                else
                {
                    hasProj = PrecipitationController::GetD3DProj(matProj);
    ...
    // MW: force view-proj from view object every frame (sub_6CF400 writes this+0x80).
    if (detected_game == GameType::MW)
    {
        static bool loggedEViewVP = false;
        void* viewPtr = g_mwActiveViewPtr;
        if (!viewPtr)
            viewPtr = *reinterpret_cast<void**>(WeatherGameAddresses::EViewCurrentPtr_MW);
        if (viewPtr && core::IsReadable(viewPtr, sizeof(void*)))
        {
            void* viewObj = *reinterpret_cast<void**>(viewPtr);
            if (viewObj && core::IsReadable(reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(viewObj) + 0x80),
                                            sizeof(D3DXMATRIX)))
            {
                std::memcpy(&matViewProj, reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(viewObj) + 0x80),
                            sizeof(D3DXMATRIX));
                useViewProjOnly = true;
                hasProj = true;
                // eView VP is already in D3DX row-major form; don't transpose.
                transposeVP = false;
                if (!loggedEViewVP)
                {
                    DebugLogMatrixNoGuard("Render3D eView VP", matViewProj);
                    loggedEViewVP = true;
                }
            }
        }
